...
RESULT_PATH = LESSON_DIR / "输出" / "idempotency_result.json"
...
async def main() -> None:
    ...
    # 重试时必须沿用同一个 key。默认值是 cancel-A1001-001。
    ...
    try:
        ...
        result = {
            ...
            "tool_result": readable_tool_result(tool_result),
        }
        print(f"工具返回：{result['tool_result']}")
    except TimeoutError:
        result = {
            "outcome": "client_timed_out",
            "message": f"客户端等待 {timeout_seconds} 秒后超时。不要换幂等键，重试时仍使用 {idempotency_key}。",
        }
