...
当前 `app/聊天问答/rag_cli.py` 的父子 RAG 使用 `source/长文档示例.md` 作为原始资料，使用 `indexes/父子知识库索引.json` 作为检索索引。修改原始资料后，运行：

```powershell
py .\RAGDemo\app\知识库构建\build_parent_child_index.py
```

构建完成后再重新启动 `app/聊天问答/rag_cli.py`。如果增加或删除 Markdown 二级标题章节，也要同步检查 `indexes/父子知识库元数据.json` 的 `parent_1`、`parent_2` 等编号。
